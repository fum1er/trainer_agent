...
if "user" not in st.session_state or not st.session_state.get("strava_connected"):
    ...
    if st.session_state.get("show_auth_url"):
        ...
        st.warning(
            "Step 2: After authorizing, copy the 'code' parameter from the redirect URL and paste it below"
        )
        st.code("Example URL: http://localhost:8501/strava_callback?code=XXXXX&scope=...\nCopy only the XXXXX part", language="text")
